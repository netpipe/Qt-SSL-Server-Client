...
{
    ensure_gnutls_init();
    d->cerrno = gnutls_x509_crt_init(&d->crt);
}
...
{
    return QString::fromUtf8(gnutls_strerror(d->cerrno));
}
...
{
    d->cerrno = gnutls_x509_crt_set_crq(d->crt, crq.d->crq);
    return GNUTLS_E_SUCCESS == d->cerrno;
}
...
{
    d->cerrno = gnutls_x509_crt_set_version(d->crt, version);
    return GNUTLS_E_SUCCESS == d->cerrno;
}
...
{
    d->cerrno = gnutls_x509_crt_set_serial(d->crt, serial.constData(), serial.size());
    return GNUTLS_E_SUCCESS == d->cerrno;
}
...
{
    d->cerrno = gnutls_x509_crt_set_activation_time(d->crt, date.toTime_t());
    return GNUTLS_E_SUCCESS == d->cerrno;
}
...
{
    d->cerrno = gnutls_x509_crt_set_expiration_time(d->crt, date.toTime_t());
    return GNUTLS_E_SUCCESS == d->cerrno;
}
...
{
    d->cerrno = gnutls_x509_crt_set_crq_extensions(d->crt, crq.d->crq);
    return GNUTLS_E_SUCCESS == d->cerrno;
}
...
{
    d->cerrno = gnutls_x509_crt_set_basic_constraints (d->crt, ca, pathLength);
    return GNUTLS_E_SUCCESS == d->cerrno;
}
...
{
    d->cerrno = gnutls_x509_crt_set_key_purpose_oid(d->crt, oid.constData(), critical);
    return GNUTLS_E_SUCCESS == d->cerrno;
}
...
bool CertificateBuilder::setKeyUsage(KeyUsageFlags usages)
{
    ...
    d->cerrno = gnutls_x509_crt_set_key_usage(d->crt, usage);
    return GNUTLS_E_SUCCESS == d->cerrno;
}
...
bool CertificateBuilder::addSubjectKeyIdentifier()
{
    ...
    d->cerrno = gnutls_x509_crt_get_key_id(d->crt, 0, reinterpret_cast<unsigned char *>(ba.data()), &size);
    if (GNUTLS_E_SUCCESS != d->cerrno)
        return false;

    d->cerrno = gnutls_x509_crt_set_subject_key_id (d->crt, ba.constData(), size);
    return GNUTLS_E_SUCCESS == d->cerrno;
}
...
{
    gnutls_x509_crt_t cacrt = qsslcert_to_crt(qcacert, &d->cerrno);
    if (GNUTLS_E_SUCCESS != d->cerrno)
        return false;
    ...
    d->cerrno = gnutls_x509_crt_get_subject_key_id(cacrt, reinterpret_cast<unsigned char *>(ba.data()), &size, NULL);

    // Or fallback to creating it
    if (GNUTLS_E_SUCCESS != d->cerrno) {
        d->cerrno = gnutls_x509_crt_get_key_id(cacrt, 0, reinterpret_cast<unsigned char *>(ba.data()), &size);
    ...
    gnutls_x509_crt_deinit(cacrt);
    d->cerrno = gnutls_x509_crt_set_authority_key_id(d->crt, reinterpret_cast<const unsigned char *>(ba.constData()), size);
...
{
    gnutls_x509_privkey_t key = qsslkey_to_key(qkey, &d->cerrno);
    if (GNUTLS_E_SUCCESS != d->cerrno) {
    ...
    d->cerrno = gnutls_privkey_init(&abstractKey);
    if (GNUTLS_E_SUCCESS != d->cerrno) {
        gnutls_x509_privkey_deinit(key);
        return QSslCertificate();
    }
    ...
    d->cerrno = gnutls_x509_crt_privkey_sign(d->crt, d->crt, abstractKey, GNUTLS_DIG_SHA1, 0);
    ...
    return crt_to_qsslcert(d->crt, &d->cerrno);    
}
...
QSslCertificate CertificateBuilder::signedCertificate(const QSslCertificate &qcacert, const QSslKey &qcakey)
{
    ...
    // Extract the CA key
    //
    gnutls_x509_privkey_t key = qsslkey_to_key(qcakey, &d->cerrno);
    if (GNUTLS_E_SUCCESS != d->cerrno) {
        gnutls_x509_privkey_deinit(key);
        return QSslCertificate();
    };
    ...
    // Sign the cert
    //
    d->cerrno = gnutls_x509_crt_privkey_sign(d->crt, cacrt, abstractKey, GNUTLS_DIG_SHA1, 0);
