...
QSslKey KeyBuilder::generate( QSsl::KeyAlgorithm algo, KeyStrength strength )
{
    ...
    int aerrno = gnutls_x509_privkey_generate(key, (algo == QSsl::Rsa) ? GNUTLS_PK_RSA : GNUTLS_PK_DSA, bits, 0);
    if (GNUTLS_E_SUCCESS != aerrno) {
        qWarning("Failed to generate key %s", gnutls_strerror(aerrno));
        gnutls_x509_privkey_deinit(key);
        return QSslKey();
    }

    QSslKey qkey = key_to_qsslkey(key, algo, &aerrno);
    if (GNUTLS_E_SUCCESS != aerrno) {
        qWarning("Failed to convert key to bytearray %s", gnutls_strerror(aerrno));
        gnutls_x509_privkey_deinit(key);
        return QSslKey();
    }
