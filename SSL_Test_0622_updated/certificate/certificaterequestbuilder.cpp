...
{
    d->cerrno = gnutls_x509_crq_set_version(d->crq, version);
...
bool CertificateRequestBuilder::setKey(const QSslKey &qkey)
{
    ...
    d->cerrno = gnutls_x509_crq_set_key(d->crq, key);
...
{
    d->cerrno = gnutls_x509_crq_set_dn_by_oid(d->crq, oid.constData(), raw,
...
bool CertificateRequestBuilder::addSubjectAlternativeNameEntry(QSsl::AlternativeNameEntryType qtype, const QByteArray &value)
{
    ...
    d->cerrno = gnutls_x509_crq_set_subject_alt_name(d->crq, type, value.constData(), value.size(), GNUTLS_FSAN_APPEND);
    return GNUTLS_E_SUCCESS == d->cerrno;
}
#else
...
CertificateRequest CertificateRequestBuilder::signedRequest(const QSslKey &qkey)
{
    ...
    d->cerrno = gnutls_x509_crq_sign2(d->crq, key, GNUTLS_DIG_SHA1, 0);
    gnutls_x509_privkey_deinit(key);
