...
gnutls_x509_privkey_t qsslkey_to_key(const QSslKey &qkey, int *perrno)
{
    gnutls_x509_privkey_t key;

    *perrno = gnutls_x509_privkey_init(&key);
    ...
    *perrno = gnutls_x509_privkey_import(key, &buffer, GNUTLS_X509_FMT_PEM);
    return key;
}

gnutls_x509_crt_t qsslcert_to_crt(const QSslCertificate &qcert, int *perrno)
{
    gnutls_x509_crt_t cert;

    *perrno = gnutls_x509_crt_init(&cert);
    if (GNUTLS_E_SUCCESS != *perrno)
        return 0;
    ...
    // Import the cert
    *perrno = gnutls_x509_crt_import(cert, &buffer, GNUTLS_X509_FMT_PEM);
    return cert;
}

QSslCertificate crt_to_qsslcert(gnutls_x509_crt_t crt, int *perrno)
{
    QByteArray ba(4096, 0);
    size_t size = ba.size();

    *perrno = gnutls_x509_crt_export(crt, GNUTLS_X509_FMT_PEM, ba.data(), &size);
    if (GNUTLS_E_SUCCESS != *perrno)
...
QSslKey key_to_qsslkey(gnutls_x509_privkey_t key, QSsl::KeyAlgorithm algo, int *perrno)
{
    QByteArray ba(4096, 0);
    size_t size = ba.size();

    *perrno = gnutls_x509_privkey_export(key, GNUTLS_X509_FMT_PEM, ba.data(), &size);
    if (GNUTLS_E_SUCCESS != *perrno)
        return QSslKey();
