 ...
 */
static QByteArray request_to_bytearray(gnutls_x509_crq_t crq, gnutls_x509_crt_fmt_t format, int *perrno)
{
    QByteArray ba(4096, 0);
    size_t size = ba.size();

    *perrno = gnutls_x509_crq_export(crq, format, ba.data(), &size);

    if (GNUTLS_E_SUCCESS != *perrno)
        return QByteArray();
    ...
    : d(new CertificateRequestPrivate)
{
    ...
    d->cerrno = gnutls_x509_crq_import(d->crq, &buffer, (QSsl::Pem == format) ? GNUTLS_X509_FMT_PEM : GNUTLS_X509_FMT_DER);
...
QList<QByteArray> CertificateRequest::nameEntryAttributes()
{
    ...
    do {
        ...
        d->cerrno = gnutls_x509_crq_get_dn_oid(d->crq, index, buffer.data(), &size);
        ...
        index++;
    } while(GNUTLS_E_SUCCESS == d->cerrno);
...
QStringList CertificateRequest::nameEntryInfo(const QByteArray &oid)
{
    ...
    do {
        ...
        d->cerrno = gnutls_x509_crq_get_dn_by_oid(d->crq, oid.constData(), index, false, buffer.data(), &size);
...
{
    return request_to_bytearray(d->crq, GNUTLS_X509_FMT_PEM, &d->cerrno);
}
...
{
    return request_to_bytearray(d->crq, GNUTLS_X509_FMT_DER, &d->cerrno);
}
...
QString CertificateRequest::toText()
{
    ...
    d->cerrno = gnutls_x509_crq_print(d->crq, GNUTLS_CRT_PRINT_FULL, &datum);
